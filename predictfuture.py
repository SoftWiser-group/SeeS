...
class Sees_model(nn.Module):
    ...
    def get_future_batch(self, pdata):
        c_i, c_j = pdata[0].shape[0], pdata[0].shape[1]
        
        for s_i in range(c_i):
    ...
    def get_future_test(self, this_batch):
        src, src_mask, trg, walk, walk_src = this_batch
        src = self.forward(src, src_mask, walk, walk_src)
        src = src.view(src.shape[0]).cpu().clone().detach().numpy()
        trg = trg.view(trg.shape[0]).cpu().clone().detach().numpy()
        return src, trg
    ...
    def run_testing(self):
        ...
        for this_batch in batch_data:
            batch_num += 1
            test_b = self.get_future_test(this_batch)
            test[0].append(test_b[0])
            test[1].append(test_b[1])
